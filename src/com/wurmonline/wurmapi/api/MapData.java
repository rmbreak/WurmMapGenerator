...
public final class MapData {
  ...
  /**
   * Currently map width and height are always equal, but two methods exists in
   * case if this will change in a future.
   * 
   * @return map width
   */
  public int getWidth() {
   ...
   * @param x x location in game world.
   * @param y y location in game world.
   * @return height of dirt layer in location, or 0 if negative (rock layer being
   *         higher than surface layer).
   */
   ...
   * @param x      x location in game world.
   * @param y      y location in game world.
   * @param height height of tile, be careful with very high or very low values as
   *               both seem to cause server to crash.
   */
   ...
   * Please see
   * {@link #setSurfaceTile(int, int, com.wurmonline.mesh.Tiles.Tile, short) this
   * method} for more details.
   * 
   * @param x        x location in game world.
   * @param y        y location in game world.
   * @param tileType type of tile. Using cave, trees and bushes constants is not
   *                 allowed.
   */
  ...
  /**
   * Sets tile data on a surface.<br>
   * While it is possible to place practically everything using this method, it is
   * recommended to set things like trees and bushes using specialized methods
   * instead of this one.<br>
   * <br>
   * 
   * If tile is on lower height than rock layer height at the moment of saving map
   * to file, it will be automatically set to rock layer height.<br>
   * If the whole tile is exposed (rock level equal to surface level in all four
   * corners), its type will be changed to rock on map save.<br>
   * This method will NOT prevent doing illogical things like placing trees or
   * grass under water.<br>
   * <br>
   * 
   * Under the hood, tile data is 32-bit value, where:<br>
   * Bits 1-8 : tile type data<br>
   * Bits 9-16 : additional data (used for example by trees, bushes and grass,
   * varies from tile type to tile type, this method is not using these bits)<br>
   * Bits 17-32 : height data (as signed short value)
   * 
   * @param x        x location in game world.
   * @param y        y location in game world.
   * @param height   height of tile, be careful with very high or very low values
   *                 as both seem to cause server to crash.
   * @param tileType type of tile. Using cave, trees and bushes constants is not
   *                 allowed.
   * 
   * @see #setTree(int, int, com.wurmonline.mesh.TreeData.TreeType,
   *      com.wurmonline.mesh.FoliageAge,
   *      com.wurmonline.mesh.GrassData.GrowthTreeStage)
   * @see #setBush(int, int, com.wurmonline.mesh.BushData.BushType,
   *      com.wurmonline.mesh.FoliageAge,
   *      com.wurmonline.mesh.GrassData.GrowthTreeStage)
   */
  ...
  /**
   * Places tree in specified position in game world.<br>
   * <br>
   * 
   * Placing tree should happen after calling
   * {@link #setSurfaceTile(int, int, Tile, short) setSurfaceTile}, as it is using
   * data from a tile (height and grass type) to generate the final tree data.<br>
   * This method will NOT prevent doing illogical things like placing trees under
   * water.<br>
   * <br>
   * 
   ...
   * @param grassStage stage of grass growth on tree tile. It replaces grass
   *                   height of normal grass tile if it was previously set.
   */
  ...
  /**
   * Places bush in specified position in game world.<br>
   * <br>
   * 
   * Placing bush should happen after calling
   * {@link #setSurfaceTile(int, int, Tile, short) setSurfaceTile}, as it is using
   * data from a tile (height and grass type) to generate the final bush data.<br>
   * This method will NOT prevent doing illogical things like placing grass under
   * water.<br>
   * <br>
   * 
   * Under the hood, 1 byte of bush special data is composed in the same way as
   * tree data:
   * {@link #setTree(int, int, com.wurmonline.mesh.TreeData.TreeType, com.wurmonline.mesh.FoliageAge, com.wurmonline.mesh.GrassData.GrowthTreeStage)
   * setTree}.
   * 
   * @param x          x location in game world.
   * @param y          y location in game world.
   ...
   * @param grassStage stage of grass growth on bush tile. It replaces grass
   *                   height of normal grass tile if it was previously set.
   */
  ...
  /**
   * Sets grass data on given grass, mycelium, kelp or reed tile (for any other
   * types of tiles it does nothing).<br>
   * <br>
   * 
   * Flower type can be set ONLY for grass tiles - in other cases method will
  ...
  public void setGrass(int x, int y, GrassData.GrowthStage grassStage, GrassData.FlowerType flower) {
    ...
    surfaceMesh.setTile(x, y,
        Tiles.encode(currentHeight, (byte) currentType, GrassData.encodeGrassTileData(grassStage, grassType, flower)));
  }
  ...
  /**
   * Sets height of rock layer at location.<br>
   * <br>
   * 
   * Under the hood, rock layer is almost identical to surface layer - this method
   * sets rock as tile type, ignores tile special data and sets desired height.
   * 
   * @param x      x location in game world.
   * @param y      y location in game world.
   * @param height height of tile, be careful with very high or very low values as
   ...
   * Only solid cave walls are allowed - exception will be thrown on attempt to
   * set non-cave tile type or cave type which is not a wall.<br>
   * This method sets default values for resource counts: 51 for rock tiles and
   * 1000 for veins.
   * 
   * @param x        x location in game world.
   * @param y        y location in game world.
   ...
   * Sets tile data inside cave.<br>
   * Only solid cave walls are allowed - exception will be thrown on attempt to
   * set non-cave tile type or cave type which is not a wall.
   * 
   * @param x             x location in game world.
   * @param y             y location in game world.
   * @param tileType      type of tile. Only cave walls constants are allowed.
   * @param resourceCount number of mining actions needed to deplete vein. Must be
   *                      higher than 0.
   */
  ...
  /**
   * Sets number of mining actions needed to deplete vein in current location
   * (rock tiles count as veins as well).
   * 
   * @param x             x location in game world.
   * @param y             y location in game world.
   * @param resourceCount number of mining actions needed to deplete vein. Must be
  ...
  public void setCaveResourceCount(int x, int y, short resourceCount) {
    if (resourceCount <= 0) {
      throw new IllegalArgumentException(
          "Invalid amount of resources in cave tile: " + resourceCount + ", must be higher than 0");
    }
   ...
   * You don't need to save map first to create updated map dump - it is using
   * data from memory.
   * 
   * @return map image
   */
   ...
   * You don't need to save map first to create updated map dump - it is using
   * data from memory.<br>
   * Using this method, you can create downscaled version of image (for example,
   * if you want to take a quick look at map without creating full sized preview).
   * 
   * @param desiredPowerOfTwo desired output image power of two, or map power of
   *                          two if it is lower than this value. Must be bigger
   *                          than 7.
   * @return map image
   */
  public BufferedImage createMapDump(int desiredPowerOfTwo) {
    ...
    for (int x = 0; x < lWidth; x += downscalePower) {
      ...
      for (int y = lWidth - 1; y >= 0; y -= downscalePower) {
        ...
        float node2 = x == lWidth - 1 || y == lWidth - 1 ? node
            : (float) (getSurfaceHeight(x + downscalePower, y + downscalePower) / (Short.MAX_VALUE / 3.3f));
        ...
        final int altTarget = y / downscalePower
            - (int) (Tiles.decodeHeight(surfaceMesh.getTile(x, y)) * MAP_HEIGHT / 4 / (Short.MAX_VALUE / 3.3f))
                / downscalePower;
  ...
  /**
   * Creates flat map dump, showing all cave terrain types in different
   * colors.<br>
   * You don't need to save map first to create updated map dump - it is using
   * data from memory.
   * 
   * @param showWater set true if you want to make water visible, false otherwise.
   * @param tiles     ore types to show on cave dump (all will be shown if not
   *                  specified or null)
   * @return map image
   */
  ...
  /**
   * Creates flat map dump, showing all terrain types in different colors and with
   * contour lines.<br>
   * You don't need to save map first to create updated map dump - it is using
   * data from memory.
   * 
  ...
  public BufferedImage createTopographicDump(boolean showWater, short interval) {
    ...
    for (int x = 0; x < lWidth; x++) {
      for (int y = lWidth - 1; y >= 0; y--) {
        ...
        final short nearHeightX = x == lWidth - 1 ? height
            : Tiles.decodeHeight(surfaceMesh.getTile(x + xo + 1, y + yo));
        final short nearHeightY = y == lWidth - 1 ? height
            : Tiles.decodeHeight(surfaceMesh.getTile(x + xo, y + yo + 1));
        boolean isControur = checkContourLine(height, nearHeightNX, interval)
            || checkContourLine(height, nearHeightNY, interval) || checkContourLine(height, nearHeightX, interval)
  ...
  /**
   * Saves all changes to file. Before saving, this method will remove some map
   * errors like wrong terrain type on completely exposed tiles and surface layer
   * being lower than rock layer.
   */
